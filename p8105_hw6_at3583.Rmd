...
Summary: The cleaned birthweight dataset contains **`r nrow(birth_df)`** observations on **`r ncol(birth_df)`** variables. Key predictors include maternal characteristics (e.g., pre-pregnancy BMI, weight gain, age, race, smoking) and infant characteristics (e.g., head circumference, length, gestational age, sex). A check for missingness shows a total of **`r sum(birth_missing$n_missing)`** missing values across all variables, indicating that the dataset is effectively complete for regression modeling.
...
Summary: The proposed model uses birthweight (*bwt*) as the outcome and includes fetal size at birth (head circumference *bhead*, length *blength*), gestational age (*gaweeks*), maternal anthropometrics and behaviors (*ppbmi*, *wtgain*, *smoken*, *momage*), socioeconomic status (*fincome*), maternal race (*mrace*), and infant sex (*babysex*) as predictors. The model explains **`r round(birth_model_main_glance$r.squared, 3)`** of the variation in birthweight, indicating a strong association between these clinical and demographic factors and infant weight.
...
Summary: The residual–fitted plot shows residuals roughly centered around zero across the fitted range, with no clear curvature. Although the spread increases slightly at higher fitted birthweights, the pattern remains fairly symmetric, suggesting that a linear model with approximately constant variance provides an adequate fit for these data.
...
Summary: Cross-validated prediction errors differ meaningfully across models. The mean RMSE for the proposed model is about `r round(rmse_summary$mean_rmse[rmse_summary$model == 'Proposed model'], 1)` grams, compared to **`r round(rmse_summary$mean_rmse[rmse_summary$model == 'Length + gestational age'], 1)`** grams for the simpler length + gestational-age model and **`r round(rmse_summary$mean_rmse[rmse_summary$model == 'Head × length × sex (all interactions)'], 1)`** grams for the highly interactive model. Overall, the **`r rmse_summary$model[which.min(rmse_summary$mean_rmse)]`** yields the lowest average RMSE, indicating the best out-of-sample predictive performance while balancing flexibility and stability.
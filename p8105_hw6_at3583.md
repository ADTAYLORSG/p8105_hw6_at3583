...
age and race, is **0.43**, with a 95% confidence interval (CI) of
**(0.32**, **0.56)**.
...
``` r
# For Summary 
# For Summary (overall across all cities)
or_summary <- city_models |>
  ungroup() |>                      # <--- this is the key line
  summarise(
    min_or   = min(estimate, na.rm = TRUE),
    max_or   = max(estimate, na.rm = TRUE),
    n_gt1    = sum(conf.low > 1, na.rm = TRUE),
    n_lt1    = sum(conf.high < 1, na.rm = TRUE),
    n_cross1 = sum(conf.low <= 1 & conf.high >= 1, na.rm = TRUE)
  )
...
```
    ...
    ## # A tibble: 1 × 5
    ##   min_or max_or n_gt1 n_lt1 n_cross1
    ##    <dbl>  <dbl> <int> <int>    <int>
    ## 1  0.262   1.77     0    22       25

Summary: Across all cities, the adjusted odds ratios for solving
homicides involving male victims range from **0.26** to **1.77**. In
**0** cities, the 95% CI lies entirely above 1, indicating higher odds
of case resolution for male victims; in **22** cities, the interval lies
entirely below 1; and in **25** cities, the CI includes 1, suggesting no
clear difference by victim sex.
...
concentrated, with a median of **0.941** and a 95% interval from
**0.934** to **0.947**. This suggests that the linear model with *tmin*
and *prcp* consistently explains a moderate proportion of the
variability in *tmax* across resampled datasets.

The bootstrap distribution of the product
$\hat{\beta}_{tmin}\hat{\beta}_{prcp}$ is centered around **-0.00583**,
with a 95% interval of **(-0.00822**, **-0.00373)**. The spread and sign
of this distribution reflect both the magnitudes and directions of the
*tmin* and *prcp* coefficients, illustrating the uncertainty in their
combined contribution to predicting *tmax*.

# Problem 3 – Birthweight and Linear Models
...
birth_missing <- birth_df |>
  summarise(across(everything(), ~ sum(is.na(.)))) |>
  pivot_longer(everything(), names_to = "variable", values_to = "n_missing") |>
  arrange(desc(n_missing))
```

Summary: The cleaned birthweight dataset contains **4342** observations
on **20** variables. Key predictors include maternal characteristics
(e.g., pre-pregnancy BMI, weight gain, age, race, smoking) and infant
characteristics (e.g., head circumference, length, gestational age,
sex). A check for missingness shows a total of **0** missing values
across all variables, indicating that the dataset is effectively
complete for regression modeling.

## A. Proposed regression model

``` r
# Proposed model for birthweight
birth_model_main <- lm(
  bwt ~ babysex + bhead + blength + gaweeks +
    ppbmi + wtgain + smoken + momage + mrace + fincome,
  data = birth_df
)

birth_model_main_tidy <- tidy(birth_model_main)
birth_model_main_glance <- glance(birth_model_main)
birth_model_main_glance
```

    ## # A tibble: 1 × 12
    ##   r.squared adj.r.squared sigma statistic p.value    df  logLik    AIC    BIC
    ##       <dbl>         <dbl> <dbl>     <dbl>   <dbl> <dbl>   <dbl>  <dbl>  <dbl>
    ## 1     0.714         0.713  274.      902.       0    12 -30529. 61087. 61176.
    ## # ℹ 3 more variables: deviance <dbl>, df.residual <int>, nobs <int>

Summary: The proposed model uses birthweight (*bwt*) as the outcome and
includes fetal size at birth (head circumference *bhead*, length
*blength*), gestational age (*gaweeks*), maternal anthropometrics and
behaviors (*ppbmi*, *wtgain*, *smoken*, *momage*), socioeconomic status
(*fincome*), maternal race (*mrace*), and infant sex (*babysex*) as
predictors. The model explains **0.714** of the variation in
birthweight, indicating a strong association between these clinical and
demographic factors and infant weight.

B. Residuals–Fitted Values

``` r
# Plot for residuals vs fitted
birth_resid_df <- birth_df |>
  add_predictions(birth_model_main) |>
  add_residuals(birth_model_main)

birth_resid_df |>
  ggplot(aes(x = pred, y = resid)) +
  geom_point(alpha = 0.4, color = primary_color) +
...
![](p8105_hw6_at3583_files/figure-gfm/unnamed-chunk-11-1.png)<!-- -->

Summary: The residual–fitted plot shows residuals roughly centered
around zero across the fitted range, with no clear curvature. Although
the spread increases slightly at higher fitted birthweights, the pattern
remains fairly symmetric, suggesting that a linear model with
approximately constant variance provides an adequate fit for these data.
...
# Fit three models on each train set
cv_df <- cv_df |>
  mutate(
...
# RMSEs into long format
rmse_long <- cv_df |>
  select(rmse_main, rmse_len_ga, rmse_interact) |>
  pivot_longer(
    everything(),
    names_to = "model",
    values_to = "rmse"
  ...
  mutate(
    model = recode(
      ...
      rmse_interact = "Head × length × sex (all interactions)"
    )
  )
...
```
    ...
    ##   <chr>                                      <dbl>   <dbl>
    ## 1 Head × length × sex (all interactions)      291.   10.5 
    ## 2 Length + gestational age                    334.   15.2 
    ## 3 Proposed model                              276.    9.76
...
rmse_long |>
  ...
  scale_fill_manual(values = c(
    "Proposed model"                           = primary_color,
    "Length + gestational age"                 = accent_color,
    "Head × length × sex (all interactions)"   = "grey60"
  )) +
  labs(
...
![](p8105_hw6_at3583_files/figure-gfm/unnamed-chunk-12-1.png)<!-- -->

Summary: Cross-validated prediction errors differ meaningfully across
models. The mean RMSE for the proposed model is about 276.3 grams,
compared to **334.5** grams for the simpler length + gestational-age
model and **290.7** grams for the highly interactive model. Overall, the
**Proposed model** yields the lowest average RMSE, indicating the best
out-of-sample predictive performance while balancing flexibility and
stability.
